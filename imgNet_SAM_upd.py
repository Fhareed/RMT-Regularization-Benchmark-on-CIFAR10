...
imagenet_path = "/2tb/Farid_image_classification/ILSVRC2017_CLS-LOC/ILSVRC/Data/CLS-LOC"
...
trainset = ImageFolder(root="/2tb/Farid_image_classification/ILSVRC2017_CLS-LOC/ILSVRC/Data/CLS-LOC/train", transform=transform_train)
valset = ImageFolder(root="/2tb/Farid_image_classification/ILSVRC2017_CLS-LOC/ILSVRC/Data/CLS-LOC/val", transform=transform_val)
#testset = ImageFolder(root="/2tb/Farid_image_classification/ILSVRC2017_CLS-LOC/ILSVRC/Data/CLS-LOC/test", transform=transform_test)
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = VisRetNet( num_classes=1000, embed_dims=[64, 128, 256, 512], depths=[2, 2, 8, 2], num_heads=[4, 4, 8, 16],init_values=[2, 2, 2, 2],
...
base_optimizer = torch.optim.Adam  # can also be SGD
optimizer = SAM(model.parameters(), base_optimizer, rho=0.05, lr=0.0003)
...
def train_one_epoch_bckprp(model: torch.nn.Module, data_loader: Iterable,
                           optimizer, device: torch.device, epoch: int,
                           criterion, scheduler, train_losses, train_accuracies):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        print(f"[Batch {i}] --> Starting forward pass...")
        forward_start = time.time()
        outputs = model(images)
        loss = criterion(outputs, labels)
        print(f"[Batch {i}] Forward pass done in {time.time() - forward_start:.2f}s")

        print(f"[Batch {i}] --> Backward (1st) & first_step...")
        backward1_start = time.time()
        loss.backward()
        optimizer.first_step(zero_grad=True)
        print(f"[Batch {i}] First step done in {time.time() - backward1_start:.2f}s")

        print(f"[Batch {i}] --> Second forward pass...")
        outputs = model(images)
        loss_second = criterion(outputs, labels)

        print(f"[Batch {i}] --> Backward (2nd) & second_step...")
        backward2_start = time.time()
        loss_second.backward()
        optimizer.second_step(zero_grad=True)
        print(f"[Batch {i}] Second step done in {time.time() - backward2_start:.2f}s")
        ...
        if i % 10 == 0:
            ...
            print(f"Epoch {epoch+1}, Batch {i}, Loss: {loss.item():.4f}, Time: {batch_time:.2f}s")
            start_time = time.time()
...
def evaluate(model: torch.nn.Module, data_loader: Iterable, device: torch.device, criterion):
    model.eval()
    total = 0
    correct = 0
    total_loss = 0.0
...
for epoch in range(start_epoch, num_epochs):
    train_one_epoch_bckprp(model=model,
