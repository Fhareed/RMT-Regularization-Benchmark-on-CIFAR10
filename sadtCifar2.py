...
import copy  # Make sure this is imported
...
"""
# Step 2: Load checkpoint FIRST (still on CPU)
#checkpoint = torch.load("your_checkpoint.pth", map_location="cpu")
#model.load_state_dict(checkpoint['model'])  # or strict=False if needed

# Step 3: Move the WHOLE model to GPU
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def add_noise_to_layer(param_group, stddev=1e-3):
    for param in param_group:
        noise = torch.randn_like(param) * stddev
        param.add_(noise)  # safer than copy_ or in-place overwrite

def get_salient_block(model, block_idx=3):
    try:
        return model.layers[block_idx].blocks[0]
    except Exception as e:
        print(f"[Warning] Couldn't find salient block at index {block_idx}: {e}")
        return None


def train_one_epoch_sadt2(model, criterion, data_loader, optimizer, device, epoch, salient_block_idx=2, max_norm=0, stddev=1e-3):
    model.train()
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        loss_ce = criterion(outputs_clean, targets)
        logits_clean = F.log_softmax(outputs_clean.detach(), dim=1)

        # Save clean model state
        original_state = {k: v.clone() for k, v in model.state_dict().items()}

        # ------------- Perturb Last Layer -------------
        model_last = copy.deepcopy(model).to(device)
        model_last.eval()
        with torch.no_grad():
        ...
        with torch.no_grad():
            logits_last = F.softmax(model_last(samples), dim=1).clamp(min=1e-7)
        loss_kld1 = kldiv(logits_clean, logits_last)

        # ------------- Perturb Salient Layer -------------
        model_sal = copy.deepcopy(model).to(device)
        model_sal.eval()
        with torch.no_grad():
            salient_block = get_salient_block(model_sal, salient_block_idx)
            if salient_block:
                add_noise_to_layer(salient_block.parameters(), stddev=stddev)

        with torch.no_grad():
            logits_sal = F.softmax(model_sal(samples), dim=1).clamp(min=1e-7)
        loss_kld2 = kldiv(logits_clean, logits_sal)

        # ------------- Final Loss + Backward -------------
        loss_total = loss_ce + loss_kld1 + loss_kld2
        optimizer.zero_grad()
        loss_total.backward()
        if max_norm > 0:
...
for epoch in range(num_epochs):
    train_loss, train_acc = train_one_epoch_sadt2(
        model=model,
        criterion=criterion,
        ...
        device=device,
        epoch=epoch,
        salient_block_idx=2,  # 3 = last layer; change to 2 or 1 for deeper perturbation
	stddev=1e-3
    )
