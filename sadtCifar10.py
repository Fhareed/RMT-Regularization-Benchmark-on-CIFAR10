...
def train_one_epoch_bckprp(
    ...
    """
    One training epoch with optional SADT:
      1) Supervised update (clean weights)
      2) KD update between clean logits (teacher) and noisy logits (student),
         with noise removed after the update.
    """
    model.train()
    running_loss = 0.0
    running_kd = 0.0
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        # -------- (1) Supervised step on clean weights --------
        optimizer.zero_grad(set_to_none=True)
        outputs = model(samples)
        loss = criterion(outputs, targets)

        if not torch.isfinite(loss):
            print(f"Non-finite loss at epoch {epoch}, batch {idx}. Stopping.")
            break
        ...
        # Track train accuracy on clean pass
        running_loss += loss.item()
        _, predicted = torch.max(outputs.data, 1)
        total += targets.size(0)
        correct += (predicted == targets).sum().item()

        # -------- (2) SADT KD step (optional) --------
        if use_sadt:
            # 2.0 Save clean weights AFTER supervised update
            with torch.no_grad():
                clean_params = [p.detach().clone() for p in model.parameters()]

            # 2.1 Teacher logits from clean weights
            with torch.no_grad():
                logits_clean = model(samples).detach()

            # 2.2 Add Gaussian noise (and store noisy baseline)
            with torch.no_grad():
                noisy_before = [p.detach().clone() for p in model.parameters()]
                for p in model.parameters():
                    if p.requires_grad and p.dim() > 1:
                        p.add_(torch.randn_like(p) * noise_std)

            # 2.3 Forward with noisy params
            logits_noisy = model(samples)

            # 2.4 KD loss: KL(teacher || student)
            T = kd_temperature
            with torch.no_grad():
                teacher_probs = F.softmax(logits_clean / T, dim=1)
            student_logprobs = F.log_softmax(logits_noisy / T, dim=1)
            kd_loss = (T * T) * kldiv(student_logprobs, teacher_probs) * kd_scale

            # 2.5 Backprop & step on noisy params
            optimizer.zero_grad(set_to_none=True)
            kd_loss.backward()
            optimizer.step()

            # 2.6 Remove injected noise but keep KD update
            with torch.no_grad():
                for p, p_clean, p_noisy_before in zip(model.parameters(), clean_params, noisy_before):
    ...
    avg_kd = (running_kd / len(data_loader)) if use_sadt else 0.0
    accuracy = 100.0 * correct / total

    if use_sadt:
        print(f"Epoch [{epoch+1}] completed. "
              f"Avg Loss: {avg_loss:.4f}, KD Loss: {avg_kd:.4f}, Accuracy: {accuracy:.2f}%")
    else:
        print(f"Epoch [{epoch+1}] completed. "
...
# Replace or wrap your existing loss function for compatibility
...
for epoch in range(num_epochs):
    train_loss, train_acc = train_one_epoch_bckprp(
        model=model,
        criterion=criterion,
        ...
        epoch=epoch,
        # --- SADT knobs ---
        use_sadt=True,         # set False to disable
        noise_std=5e-5,        # try 5e-5 … 5e-4
        kd_temperature=1.0,    # 1.0 usually fine; 2–4 to soften targets
        kd_scale=1.0           # weight of the KD step
    )
