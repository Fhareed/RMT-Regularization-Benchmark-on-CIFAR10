...
Original file is located at
    https://colab.research.google.com/drive/1B9_UjBrT45Mtz0vTuunWQWZH2QG_zqWq
"""
...
train_transform = transforms.Compose([
   ...
   # transforms.AutoAugment(policy=transforms.AutoAugmentPolicy.CIFAR10),
    transforms.ToTensor(),
    #transforms.RandomErasing(p=0.10, scale=(0.02, 0.2), ratio=(0.3, 3.3)),
...
# epoch-wise cosine annealing (no per-batch stepping required)
num_epochs = 100
...
def cutmix_data(x, y, alpha=1.0):
    ''' x: images, y: labels (class indices) '''
...
def train_one_epoch_bckprp(model, criterion, data_loader, optimizer, device, epoch,
                           max_norm=0, use_cutmix=True, cutmix_prob=0.2, cutmix_alpha=1.0):
    model.train()
    running_loss = 0.0
    # use float accumulators for weighted acc
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        is_cm = use_cutmix and np.random.rand() < cutmix_prob
        if is_cm:
            samples, targets_a, targets_b, lam = cutmix_data(samples, targets, alpha=cutmix_alpha)
        ...
        with torch.no_grad():
            ...
            if is_cm:
                # weighted by lam (scalar here); if you make lam per-sample later, compute per-sample weights
                acc_batch = lam * (pred == targets_a).float() + (1.0 - lam) * (pred == targets_b).float()
                correct_weighted += acc_batch.sum().item()
                total += targets.size(0)
            else:
                correct_weighted += (pred == targets).float().sum().item()
                total += targets.size(0)

    avg_loss = running_loss / len(data_loader)
    accuracy = 100.0 * correct_weighted / max(1, total)
