...
def add_noise_to_weights(model, stddev=1e-3):
    with torch.no_grad():
...
def train_one_epoch_sadt(model, criterion, data_loader, optimizer, device, epoch, max_norm=0):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    # KL divergence as used in TF version
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        # Save base logits (teacher)
        logits_clean = F.log_softmax(outputs.detach(), dim=1)

        # === 2. Backward pass for base gradients ===
        optimizer.zero_grad()
        loss.backward()
        grads_base = [p.grad.clone() if p.grad is not None else None for p in model.parameters()]

        # === 3. Save original weights ===
        original_weights = [p.data.clone() for p in model.parameters()]

        # === 4. Perturb weights with Gaussian noise ===
        add_noise_to_weights(model)

        # === 5. Forward pass with perturbed weights ===
        outputs_perturbed = model(samples)
        logits_perturbed = F.softmax(outputs_perturbed, dim=1)

        # === 6. KL divergence loss between teacher and student ===
        loss_kld = kldiv(logits_clean, logits_perturbed)
        ...
        # === 8. Backward pass for KL gradients ===
        optimizer.zero_grad()
        loss_kld.backward()
        grads_kld = [p.grad.clone() if p.grad is not None else None for p in model.parameters()]

        # === 9. Apply combined gradient update (TF logic: grads_kld + 0.5 * grads_base) ===
        for p, g_base, g_kld in zip(model.parameters(), grads_base, grads_kld):
            if g_base is not None and g_kld is not None:
                p.grad = g_kld + 0.5 * g_base
            elif g_kld is not None:
                p.grad = g_kld

        # Optional: clip gradient norms
        if max_norm > 0:
...
for epoch in range(num_epochs):
    train_loss, train_acc = train_one_epoch_sadt(
        model=model,
        criterion=criterion,
