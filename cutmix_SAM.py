...
train_transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    # Gentle erasing; adjust if under/overfitting
...
print(f"[LOG] Datasets ready. Train size={len(trainset)}, Test size={len(testset)}")

print("[LOG] Creating dataloaders...")
# If stuck, temporarily set num_workers=0
...
def cutmix_data(x, y, alpha=1.0):
    """ x: images (B,C,H,W), y: class indices """
...
# -------------- FIRST-BATCH SANITY --------------
print("[LOG] Fetching first batch to validate pipeline...")
t0 = time.time()
try:
    xb, yb = next(iter(trainloader))
    dt = time.time() - t0
    print(f"[LOG] First batch OK: x={tuple(xb.shape)}, y={tuple(yb.shape)} in {dt:.2f}s")
except Exception as e:
    print("[ERROR] Failed to fetch first batch from trainloader!")
    raise

# ----- Train/Eval for SAM with CutMix -----
def train_one_epoch_sam(model, criterion, trainloader, optimizer, device, epoch,
                        use_cutmix=True, cutmix_prob=0.3, cutmix_alpha=1.0, max_norm=0.0):
    model.train()
    running_loss = 0.0; correct = 0; total = 0
    for idx, (images, labels) in enumerate(trainloader):
        if idx % 20 == 0:
            print(f"[LOG] E{epoch} B{idx}: batch received")

        images, labels = images.to(device, non_blocking=True), labels.to(device, non_blocking=True)

        do_cutmix = (use_cutmix and np.random.rand() < cutmix_prob)
        if do_cutmix:
            images, targets_a, targets_b, lam = cutmix_data(images, labels, alpha=cutmix_alpha)
            if idx % 20 == 0:
                print(f"[LOG] E{epoch} B{idx}: cutmix applied (lam≈{lam:.3f})")

        # ------- First SAM step -------
        outputs = model(images)
        if idx % 20 == 0:
            print(f"[LOG] E{epoch} B{idx}: forward1 ok, out={tuple(outputs.shape)}")

        loss = (lam * criterion(outputs, targets_a) + (1. - lam) * criterion(outputs, targets_b)) if do_cutmix \
               else criterion(outputs, labels)
        loss.backward()
        if idx % 20 == 0:
            print(f"[LOG] E{epoch} B{idx}: backward1 ok, loss={loss.item():.4f}")
        ...
        if idx % 20 == 0:
            print(f"[LOG] E{epoch} B{idx}: first_step ok")

        # ------- Second SAM step -------
        outputs = model(images)
        if idx % 20 == 0:
            print(f"[LOG] E{epoch} B{idx}: forward2 ok")

        loss_second = (lam * criterion(outputs, targets_a) + (1. - lam) * criterion(outputs, targets_b)) if do_cutmix \
                      else criterion(outputs, labels)
        loss_second.backward()
        if idx % 20 == 0:
            print(f"[LOG] E{epoch} B{idx}: backward2 ok, loss2={loss_second.item():.4f}")
        ...
        if idx % 20 == 0:
            print(f"[LOG] E{epoch} B{idx}: second_step ok")

        running_loss += loss.item()
        _, pred = torch.max(outputs.data, 1)
        total += labels.size(0)
        correct += (pred == (targets_a if do_cutmix else labels)).sum().item()
...
def evaluate(model, criterion, testloader, device):
    model.eval()
    total = 0; correct = 0; test_loss = 0.0
...
# ---------------- OPTIM / SCHED / LOSS ----------------
print("[LOG] Building loss, optimizer, scheduler...")
...
# ---------------- TRAIN LOOP with LOGS ----------------
train_losses, train_accuracies = [], []
val_losses, val_accuracies = [], []

global_start = time.time()
print("[LOG] ===== Training start =====")

for epoch in range(num_epochs):
    epoch_start = time.time()

    # Use CutMix for first 90 epochs; off for the last 10 (adjust if you prefer)
    use_cm = (epoch < num_epochs - 10)

    print(f"\n[LOG] Epoch {epoch+1}/{num_epochs} | use_cutmix={use_cm} | "
          f"current_lr={optimizer.param_groups[0]['lr']:.6f}")
    ...
    train_loss, train_acc = train_one_epoch_sam(
        ...
        cutmix_prob=0.3,
        cutmix_alpha=1.0,
        max_norm=1.0,   # grad clipping inside function (no structure change)
    )
    t_train = time.time() - t_train0
    print(f"[LOG] Train done in {t_train:.2f}s | loss={train_loss:.4f}, acc={train_acc:.2f}%")

    # --- Eval ---
    t_eval0 = time.time()
    val_loss, val_acc = evaluate(model, criterion, testloader, device)
    t_eval = time.time() - t_eval0
    print(f"[LOG] Eval  done in {t_eval:.2f}s | val_loss={val_loss:.4f}, val_acc={val_acc:.2f}%")
...
# -------------- GLOBAL TIMING FOOTER --------------
end_time = time.time()
elapsed = end_time - global_start
